...
\title{\large{Autoformalizing Fast--Slow Equivalence Proofs for Atlas Computations in the Unitary Dual Problem}}


\author{\normalsize{Mario Carneiro\inst{1} \and 
   Stephen D. Miller\inst{2} \and 
   Magnus Myreen\inst{1} \and
   Josef Urban\inst{1,3} 
 }}
\institute{
$^1$Chalmers University and University of Gothenburg \qquad
$^2$Rutgers University \qquad
$^3$AI4REASON
}


\authorrunning{Carneiro, Miller, Myreen, Urban}
...
\paragraph{Goal} The goal of this \href{https://github.com/JUrban/atlasofliegroups_sml1/tree/to_ht_branch_alt}{project} is the formal verification of optimized computations arising in the
unitary dual problem.  The motivating case is an
\atlas{} workflow for determining all  unitary representations of a Lie group with infinitesimal character in the
fundamental parallelepiped.  The production code uses many
hand-written optimizations, implemented partly in the \axis{} scripting layer,
while a slower brute-force checker gives a simpler reference computation. We report on an ongoing
\texttt{.at}-to-\sml translation and a staged \holfour{}/\cakeml verification
stack (about \href{https://github.com/jeffreyadams/atlasofliegroups/compare/to_ht_branch_alt...JUrban:atlasofliegroups_sml1:to_ht_branch_alt}{500 commits}) aimed at proving this fast--slow equivalence for the case of the Lie group $F_{4(4)}$, and
scaling  to  larger computations of interest such as for $E_{8(8)}$. 
...
\paragraph{Our plan: replacement architecture in SML - coded by Codex}
We \href{https://github.com/JUrban/atlasofliegroups_sml1/tree/to_ht_branch_alt}{replace} (in almost automated Codex sessions) \axis{} (\texttt{.at}) scripts by \sml programs using \polyml{} \cite{PolyML}.  A C
ABI shim exposes a curated subset of the \atlas{} C++ API as a shared library,
which is then bound in \sml using the \polyml \href{https://github.com/JUrban/atlasofliegroups_sml1/blob/to_ht_branch_alt/atlas-scripts-sml/ffi}{foreign-function interface}.  The
...
available for translation to \cakeml{} \cite{CakeML} and reasoning/verification in \holfour{} \cite{HOL4}, while keeping
...
% proof development.
The point is not to replace \atlas{} mathematics, but to
put the fragile optimization layer into a form where equivalence theorems can
be stated, generated, reviewed, and ultimately proved. The Codex coding was supervised by us and used unit testing based on $F_4$ etc.

\paragraph{Case study: $F_4$ FPP unitary dual verification}
Our principal target is the \atlas{} script
\href{https://github.com/JUrban/atlasofliegroups_sml1/blob/to_ht_branch_alt/atlas-scripts/script_to_verify_F4_FPP_unitary_dual.at}{\texttt{script\_to\_verify\_F4\_FPP\_unitary\_dual.at}}.  It checks a finite set
...
Our Codex-translated \sml entry point
\href{https://github.com/JUrban/atlasofliegroups_sml1/blob/to_ht_branch_alt/atlas-scripts-sml/script_to_verify_F4_FPP_unitary_dual.sml}{\texttt{script\_to\_verify\_F4\_FPP\_unitary\_dual.sml}} runs a fast pipeline
...
The corresponding slow program, \href{https://github.com/JUrban/atlasofliegroups_sml1/blob/to_ht_branch_alt/atlas-scripts-sml/SimplerVerifyF4FPP.sml}{\texttt{SimplerVerifyF4FPP.sml}} is a direct
translation of a \href{https://github.com/JUrban/atlasofliegroups_sml1/blob/to_ht_branch_alt/atlas-scripts/simpler_script_to_verify_F4_FPP_unitary_dual.at}{brute-force checker} that ranges over a large triple domain of
...
been proved and what remains as an explicit contract.
\iffalse
This is also where LLM
...
\section{Translator plan: discharging imperative obligations in \cakeml}
